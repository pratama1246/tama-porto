...
export const Card = forwardRef(({ customClass, ...rest }, ref) => (
  <div
    ref={ref}
    {...rest}
    className={`absolute top-1/2 left-1/2 rounded-[4px] border border-black/10 bg-white [transform-style:preserve-3d] [will-change:transform] [backface-visibility:hidden] ${customClass ?? ''} ${rest.className ?? ''}`.trim()}
  />
));
...
const CardSwap = ({
  ...
  pauseOnHover = false,
  onCardClick,
  onIndexChange, // Callback to notify parent of the current front card index
  skewAmount = 6,
  easing = 'elastic',
  containerClassName = '',
  children
  ...
  useEffect(() => {
    ...
    const swap = () => {
      ...
      // Prevent concurrent animations from running at the same time
    ...
    // Do not call swap() immediately on mount, so card 0 is displayed for the duration of the first interval
